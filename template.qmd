...
library(marquee)
set_unifrog_geoms()
theme_set(unifrog_theme(grid = 'Y', axis = 'x'))
library(ragg)
ragg_png = function(..., res = 72) {
  ragg::agg_png(..., res = res, units = "in")
}
knitr::opts_chunk$set(dev = "ragg_png", fig.ext = "png", fig.width = 7, fig.height = 4)
dev.args = list(ragg_png = list(res = 72))
```
...
For help connecting to Azure checkout \[this
vignette\].(https://ishmaelroslan.github.io/UnifrogTools/articles/Azure.html)

For help on themeing plots see this \[vignette\].(https://ishmaelroslan.github.io/UnifrogTools/articles/Palettes.html)

## Custom Classes

For highlighting words, there are custom classes, which set the text to bold and give the text the specified unifrog
colour. In the example below the text orange has the `.orange` class. It is applied in markdown like so:
`[this text is orange]{.orange}`. The available colours are shown below, with some examples
